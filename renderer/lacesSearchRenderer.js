...
function renderTable() {
    ipcRenderer.send('get-search-laces', null);
}

ipcRenderer.on('search-laces', (event, arg) => {
    const { laces, colors, suppliers } = arg;
    ...
    showTable(laces, colors, suppliers);
    ...
    apply_filters.addEventListener('click', () => {
        const filteredlaces = laces.filter(lace => {
            return (!code || lace.code.includes(code)) &&
                   (!name || lace.name.includes(name)) &&
                   (!date || lace.date.includes(date)) &&
                   (selectedColors.length === 0 || selectedColors.includes(lace.color_id)) &&
                   (selectedSuppliers.length === 0 || selectedSuppliers.includes(lace.supplier_id));
        });
    
        showTable(filteredlaces, colors, suppliers);
    });
...
function makeCellEditable(cell) {
    ...
    input.addEventListener('keydown', (event) => {
        if (event.key === 'Enter') {
            ...
            ipcRenderer.send('update-search', { column, value: input.value, id, table: 'laces' });
...
function makeCellEditableSelect(cell, colors, suppliers) {
    ...
    select.addEventListener('keydown', (event) => {
        if (event.key === 'Enter') {
            ...
            ipcRenderer.send('update-search', { column, value: select.value, id, table: 'laces' });
        } else if (event.key === 'Escape') {
...
function showTable(laces, colors, suppliers) {
    ...
    laces.forEach((lace) => {
        ...
        row.innerHTML = `
            ...
            <td style="white-space: nowrap;" class="py-3 px-5 border-b-2 border-gray-600" data-column="code" data-id="${lace.id}">${lace.code}</td>
            <td style="white-space: nowrap;" class="py-3 px-5 border-b-2 border-gray-600" data-column="name" data-id="${lace.id}">${lace.name}</td>
            <td style="white-space: nowrap;" class="py-3 px-5 border-b-2 border-gray-600" data-select="color_id" data-id="${lace.id}">${lace.color}</td>
            <td style="white-space: nowrap;" class="py-3 px-5 border-b-2 border-gray-600" data-column="purchaseprice" data-id="${lace.id}">${lace.purchaseprice}</td>
            <td style="white-space: nowrap;" class="py-3 px-5 border-b-2 border-gray-600" data-column="sellingprice" data-id="${lace.id}">${lace.sellingprice}</td>
            <td style="white-space: nowrap;" class="py-3 px-5 border-b-2 border-gray-600" data-column="length" data-id="${lace.id}">${lace.length}</td>
            <td style="white-space: nowrap;" class="py-3 px-5 border-b-2 border-gray-600" data-select="supplier_id" data-id="${lace.id}">${lace.supplier}</td>
            <td style="white-space: nowrap;" style="white-space: nowrap;" class="py-3 px-5 border-b-2 border-gray-600 w-64" data-column="date" data-id="${lace.id}">${lace.date}</td>
        `;
